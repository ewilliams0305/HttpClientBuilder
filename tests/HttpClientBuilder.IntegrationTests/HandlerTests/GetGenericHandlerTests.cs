...
public class GetGenericHandlerTests
{
    private readonly IHttpClient _client;
    public GetGenericHandlerTests(AppFactory factory)
    {
        // Arrange
    ...
    public async Task Handler_CreatesSuccessObject_WhenRequestIsSuccessful()
    {
        ...
        var dispatch = _client.CreateGetHandler<WeatherForecast>("/", handler);
    ...
    public async Task Handler_CreatesException_WhenRequestFails()
    {
        // Arrange
        var handler = new TestingGenericHandler();
        var dispatch = _client.CreateGetHandler<WeatherForecast>("/api/weather/exception", handler);
