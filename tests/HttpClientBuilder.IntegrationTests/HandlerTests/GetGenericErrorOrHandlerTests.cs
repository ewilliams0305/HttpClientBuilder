...
public class GetGenericErrorOrHandlerTests
{
    private readonly IHttpClient _client;
    public GetGenericErrorOrHandlerTests(AppFactory factory)
    {
        // Arrange
    ...
    public async Task Handler_CreatesSuccessObject_WhenRequestIsSuccessful(int code)
    {
        ...
        var dispatch = _client.CreateGetHandler<WeatherForecast, ErrorResponse>($"/weather/error?error=false", handler);
    ...
    public async Task Handler_CreatesErrorObject_WhenRequestIsErrorCode()
    {
        // Arrange
        var handler = new TestingErrorOrHandler();
        var dispatch = _client.CreateGetHandler<WeatherForecast, ErrorResponse>($"/weather/error?error=true", handler);
    ...
    public async Task Handler_CreatesException_WhenThrows()
    {
        // Arrange
        var handler = new TestingErrorOrHandler();
        var dispatch = _client.CreateGetHandler<WeatherForecast, ErrorResponse>($"/weather/exception", handler);
